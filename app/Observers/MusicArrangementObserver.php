    ...
    public function updated(MusicArrangement $arrangement): void
    {
        ...
        if ($arrangement->isDirty('status') && $arrangement->status === 'arrangement_approved') {
            ...
            try {
                ...
                // IMPORTANT: Observer harus NEVER bergantung pada auth() (bisa null di queue/cli),
                // dan created_by tidak boleh null agar approve arrangement tidak gagal.
                $createdBy = $creativeUser
                    ?? $arrangement->reviewed_by
                    ?? $arrangement->created_by;
                if (!$createdBy) {
                    Log::warning('MusicArrangementObserver - Cannot create Creative Work: no created_by available', [
                        'arrangement_id' => $arrangement->id,
                        'episode_id' => $episode->id,
                        'creative_user_id' => $creativeUser,
                        'reviewed_by' => $arrangement->reviewed_by,
                        'arrangement_created_by' => $arrangement->created_by,
                    ]);
                    return;
                }
                ...
                if ($deadline && !$deadline->is_completed) {
                    $completedBy = $arrangement->reviewed_by ?? $arrangement->created_by;
                    if ($completedBy) {
                ...
                // Jangan biarkan observer menggagalkan approve Producer.
                Log::error('MusicArrangementObserver - Failed handling arrangement_approved', [
                    'arrangement_id' => $arrangement->id,
                    'episode_id' => $arrangement->episode_id,
                    'error' => $e->getMessage(),
                ]);
