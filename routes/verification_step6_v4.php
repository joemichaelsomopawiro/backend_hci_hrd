...
Route::get('/verify-step-6-v4', function () {
    // 1. Setup Test Data
    $program = PrProgram::firstOrCreate(
        ['name' => 'Test Program V4'],
        ['description' => 'Test Program', 'pr_dept_id' => 1, 'created_by' => 1]
    );

    $episode = PrEpisode::firstOrCreate(
        ['episode_number' => 9991, 'program_id' => $program->id],
        [
            'title' => 'Test Episode V4',
            'created_by' => 1,
...
Route::get('/cleanup-step-6-v4', function () {
    $episode = PrEpisode::where('episode_number', 9991)->first();
    if ($episode) {
    ...
    PrProgram::where('name', 'Test Program V4')->delete();
