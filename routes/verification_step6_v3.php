...
Route::get('/verify-step-6-v3', function () {
    // 1. Setup Test Data
    $program = PrProgram::firstOrCreate(
        ['name' => 'Test Program V3'],
        ['pr_dept_id' => 1, 'created_by' => 1] // Assuming dept 1 and user 1 exist
    );

    $episode = PrEpisode::firstOrCreate(
        ['episode_number' => 999, 'program_id' => $program->id],
        [
            'title' => 'Test Episode V3',
            'created_by' => 1,
    ...
    // Ensure workflow progress exists (mocking initialization if needed)
    // We strictly need step 6
    ...
    // Design Grafis: DRAFT (Should NOT trigger completion)
    try {
        PrDesignGrafisWork::updateOrCreate(
            ['pr_episode_id' => $episode->id],
            ['status' => 'draft', 'youtube_thumbnail_link' => 'http://test', 'bts_thumbnail_link' => 'http://test']
        );
    } catch (\Exception $e) {
...
Route::get('/cleanup-step-6-v3', function () {
    $episode = PrEpisode::where('episode_number', 999)->first();
    if ($episode) {
        ...
        PrEpisodeWorkflowProgress::where('pr_episode_id', $episode->id)->delete();
        $episode->delete();
    }
    PrProgram::where('name', 'Test Program V3')->delete();
